    ...
    <template v-for="(item, index) in itemsData">
      ...
      <van-dropdown-item v-if="item.gType === 2" title="位置" :ref="dropdownItemRefs">
        <div class="dropdown-bar-panel-content">
          ...
          <view-house-btns class="bottom" @clear="handleClearClick('位置')" @view-click="handleViewClick('位置')"></view-house-btns>
        </div>
      </van-dropdown-item>
      ...
      <van-dropdown-item v-if="item.gType === 1" title="筛选" :ref="dropdownItemRefs">
        <div class="dropdown-bar-panel-content">
          ...
          <view-house-btns class="bottom" @clear="handleClearClick('筛选')" @view-click="handleViewClick('筛选')"></view-house-btns>
        </div>
      </van-dropdown-item>
    </template>
  </van-dropdown-menu>
